...
async function sendOne(m: Match): Promise<void> {
  const db = getDb();
  const chat = db.prepare("SELECT telegram_chat_id FROM chats WHERE id = ?").get(m.chat_id) as
  ...
  try {
    ...
    log(
      "debug",
      `Sending notification to chat ${m.chat_id} (telegram_chat_id=${chat.telegram_chat_id}) for article ${m.article_id}, topic ${m.topic_id}`,
    );
    ...
    if (m.image_url) {
      ...
      try {
        ...
        log(
          "warn",
          `Failed to send photo by URL for article ${m.article_id}, retrying via upload: ${photoErr.message}`,
        );
        ...
        try {
          ...
          log(
            "warn",
            `Failed to send photo by upload for article ${m.article_id}, falling back to text: ${uploadErr.message}`,
          );
