...
describe("dispatchNotifications", () => {
  ...
  beforeEach(() => {
    ...
    db.prepare("INSERT INTO articles (id, feed_id, guid, url, title, summary) VALUES (1, 1, 'g1', 'https://ex.com/a', 'Title A', 'Sum A')").run();
    db.prepare("INSERT INTO articles (id, feed_id, guid, url, title, summary) VALUES (2, 1, 'g2', 'https://ex.com/b', 'Title B', 'Sum B')").run();
    db.prepare("INSERT INTO articles (id, feed_id, guid, url, title, summary) VALUES (3, 1, 'g3', 'https://ex.com/c', 'Title C', 'Sum C')").run();
    db.prepare("INSERT INTO articles (id, feed_id, guid, url, title, summary, image_url) VALUES (4, 1, 'g4', 'https://ex.com/d', 'Title D', 'Sum D', 'https://ex.com/img.jpg')").run();
  ...
  it("sends at most one notification per chat per cycle", async () => {
    ...
    const db = getDb();
    db.prepare("INSERT INTO article_topic_matches (article_id, topic_id, matched, score, checked_at, notified) VALUES (1, 1, 1, 0.8, datetime('now'), 0)").run();
    db.prepare("INSERT INTO article_topic_matches (article_id, topic_id, matched, score, checked_at, notified) VALUES (1, 3, 1, 0.7, datetime('now'), 0)").run();

    await dispatchNotifications();

    assert.equal(sendMessage.mock.callCount(), 1);

    const notified1 = db.prepare("SELECT notified FROM article_topic_matches WHERE article_id = 1 AND topic_id = 1").get() as any;
    assert.equal(notified1.notified, 1);

    const notified3 = db.prepare("SELECT notified FROM article_topic_matches WHERE article_id = 1 AND topic_id = 3").get() as any;
  ...
  it("skips and marks notified when chat_id no longer resolves", async () => {
    ...
    const db = getDb();
    db.prepare("INSERT INTO article_topic_matches (article_id, topic_id, matched, score, checked_at, notified) VALUES (1, 5, 1, 0.8, datetime('now'), 0)").run();

    await dispatchNotifications();

    const row = db.prepare("SELECT notified FROM article_topic_matches WHERE article_id = 1 AND topic_id = 5").get() as any;
  ...
  it("sendPhoto failure falls back to sendMessage after retrying via upload", async () => {
    ...
    const db = getDb();
    db.prepare("INSERT INTO article_topic_matches (article_id, topic_id, matched, score, checked_at, notified) VALUES (4, 4, 1, 0.9, datetime('now'), 0)").run();
  ...
  it("retries sendPhoto via upload when the URL-based attempt fails, without falling back to text", async () => {
    ...
    const db = getDb();
    db.prepare("INSERT INTO article_topic_matches (article_id, topic_id, matched, score, checked_at, notified) VALUES (4, 2, 1, 0.9, datetime('now'), 0)").run();
